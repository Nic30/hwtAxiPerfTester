...
class AxiPerfTesterTC(SimTestCase):
    ...
    def setUpClass(cls):
        ...
        u.DATA_WIDTH = 32
        u.MAX_BLOCK_DATA_WIDTH = 8  # to simplify sim
        cls.compileSim(u)
    ...
    def test_dump_modulo(self):
        u: AxiPerfTester = self.u
        self._sim_init_common(0x1000)
        ...
        for ch in job.channel_config:
            ...
            # (addr, delay, en)
            ch.pattern = [(0, 0, 1) for _ in range(u.RW_PATTERN_ITEMS)]
            ag = ch.addr_gen
        ...
        for ch_i, ch in enumerate(rep.channel):
            ...
            self.assertEqual(sum(1 if c > 0 else 0 for c in ch.last_values),
                             min(u.LAST_VALUES_ITEMS, 10), ch_i)
    ...
    def _sim_init_common(self, mem_size_to_init):
        u: AxiPerfTester = self.u
        tc = self
        self.setUpQueues()
        ...
        mem = self.mem
        for i in range(mem_size_to_init // (u.DATA_WIDTH // 8)):
            mem.data[i] = i

        self.procs.extend([time_sync()])

    def test_dump_exact(self):
        u: AxiPerfTester = self.u
        self._sim_init_common(0x1000)
        ...
        for ch in job.channel_config:
            ch: AxiPerfTesterChannelConfig
            # (addr, delay, en)
            ch.pattern = [(i * 4, self._rand.getrandbits(2), 1)
                          for i in range(u.RW_PATTERN_ITEMS)]
            ag = ch.addr_gen
            ag.ordering_mode = TimeDurationStorage.MODE.IN_ORDER
            ag.credit = 10
            ag.addr = 0
            ag.addr_step = 64
            ag.addr_mask = 0x1000 - 1
            ag.addr_mode = TransactionGenerator.MODE.EXACT
            ag.addr_offset = 0x0
            ag.trans_len = 0
            ag.trans_len_step = 0
            ag.trans_len_mask = 1
            ag.trans_len_mode = TransactionGenerator.MODE.EXACT
